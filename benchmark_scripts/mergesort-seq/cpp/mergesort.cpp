...
void sort(std::vector<int>& arr, int threshold) {
    std::vector<int> tmp(arr.size());
    merge_sort(arr, 0, arr.size(), tmp, threshold);
}

// Main
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cout << "Usage: bin SIZE THRESHOLD" << std::endl;
        return 1;
    }
    // Problem size
    int N = atoi(argv[1]);
    ...
    // Workers (ranks/cpuCores) not used in sequential version
