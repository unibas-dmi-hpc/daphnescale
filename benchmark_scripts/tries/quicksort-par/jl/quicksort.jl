...
function merge_sort_parallel!(arr, left, right, tmp, threshold, max_depth, depth)
    size = right - left
    if size <= 1
        return
    end
    ...
    if depth < max_depth
        left_thread = @spawn merge_sort_parallel!(arr, left, mid, tmp, threshold, max_depth, depth + 1)
        right_thread = @spawn merge_sort_parallel!(arr, mid, right, tmp, threshold, max_depth, depth + 1)
...
function sort!(arr, threshold, workers)
    tmp = copy(arr)
    max_depth = floor(Int, log2(workers))
    depth = 0
    merge_sort_parallel!(arr, 1, length(arr)+1, tmp, threshold, max_depth, depth)
end

# Main
function main()
    if length(ARGS) != 3
        println("Usage: julia mergesort.jl SIZE THRESHOLD N_WORKERS")
        exit(1)
    end

    N = parse(Int, ARGS[1])
    threshold = parse(Int, ARGS[2])
    workers = parse(Int, ARGS[3])
