...
rule all:
  input:
    ...
    #   num_threads=NUM_THREADS,\
    #   iter=ITERATIONS),   
    expand("results_tests/{matrix}/{benchmark}/{lang}314t/{num_threads}/{iter}.dat",\
...
rule run_py314t_expe_with_matrix:
  input:
    sbatch="sbatch_scripts/run_vega_py314t.sh",
  ...
  output:
    "results_tests/{matrix}/{benchmark}/{lang}314t/{num_threads}/{iter}.dat"  
  ...
  shell:
    ...
    sbatch --cpus-per-task={wildcards.num_threads} {input.sbatch} {wildcards.num_threads} {input.script} {input.mtx} {params.matrix_size} {output}
    """
